...
Discovery backtest for the three trial pairs (EURJPY, NZDJPY, CADJPY),
added 2026-08-21 as candidates -- liquid crosses pairing a European/
risk-on currency against JPY's safe-haven/funding role (or CAD/NZD
against JPY, same opposite-sentiment dynamic that makes AUDJPY trade
well). No V109 ported calibration exists for these, so their session
gate is currently the full 24h (see config.py) specifically so this
script can find where their real edge sits, same method already used
for USDCHF/AUDJPY/NZDCAD.

User's explicit plan: backtest first: only if genuinely positive, run a
one-week PAPER-ONLY live trial before ever considering a real relay.
"""
...
NEW_PAIRS = ["EURJPY", "NZDJPY", "CADJPY"]
...
async def main() -> None:
    ...
    print(f"\nTotal trades across all 3 pairs: {len(all_trades)}")
