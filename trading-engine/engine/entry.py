...
RSI_MOMENTUM_GATE_ENABLED = True  # [BACK ON 2026-09-10, explicit user
# instruction] Was OFF ~06:44-~09:4x UTC for an A/B test after a quiet
# stretch made the user suspect the gate itself was suppressing trades.
# With it off, 3 real trades fired (NZDCAD, USDCHF, EURNZD) -- all 3 had
# rsi_momentum_5m/15m already "building"/"building" on their own, meaning
# none of them would have been blocked by the gate anyway. That's
# consistent with the live diagnostics taken during the earlier quiet
# stretch (RSI was touching only 1-2 of 21 pairs at any given moment; the
# MSS structural-break gate was the real bottleneck) -- no case of the
# gate actually costing a real trade showed up in this window. Turning
# back on with that as the resolution, not because the A/B proved
# nothing -- it just didn't produce the specific counter-example needed
# to keep it off.
